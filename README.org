...
On GitHub:
1. Check the Packages section below this readme.

   The Packages section should contain instructions for pulling a
   Docker image, i.e.:
    #+begin_src sh
      $ docker pull ghcr.io/branjam4/emacs-ebmm:main
      $ # run the image with next command
      $ docker run ghcr.io/branjam4/emacs-ebmm:main
  #+end_src

2. Check the Actions tab for artifacts.

   If on Linux, a compressed archive is available to download and can
   be used as follows:
   #+begin_src sh
     $ tar -xf ebmm.tar.bz2
   ...
   #+end_src
* Example Usage
Requires PlantUML and plantuml-mode:
#+begin_src elisp
